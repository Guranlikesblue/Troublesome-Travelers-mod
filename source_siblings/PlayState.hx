...
class PlayState extends MusicBeatState
{
	...
	public static var isEvilStage:Bool = false;
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'encount' | 'more' | 'hungerstation':
					curStage = 'spooky';
				case 'devour':
					curStage = 'spookyEvil';
		...
		isEvilStage = stageData.isEvilStage;
		...
		switch (curStage)
		{
			...
			case 'spookyEvil': //Week 2
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('halloweenevil_bg', -200, -100, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('halloweenevil_bg_low', -200, -100);
				}
				add(halloweenBG);
			...
			case 'spookyEvilfront':
				var bg:BGSprite = new BGSprite('halloweenevil', -200, -100, 0, 0);
					add(bg);
			...
			case 'park': //Week 2 - Phony, Magical-Girl
				var bg:BGSprite = new BGSprite('parkBG/parkback', -1000, -300, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				if (!ClientPrefs.lowQuality) {
					var bgEscalator:BGSprite = new BGSprite('parkBG/parkbench', -1000, -300, 0.3, 0.3);
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					add(bgEscalator);
					
					upperBoppers = new BGSprite('parkBG/parkpico', -900, -225, 0.3, 0.3, ['pico idle']);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					upperBoppers.updateHitbox();
					add(upperBoppers);
				}
				
				bottomBoppers = new BGSprite('parkBG/parkbopper', -1300, -500, 0.9, 0.9, ['bopper idle']);
				bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
				bottomBoppers.updateHitbox();
				add(bottomBoppers);

				var fgSnow:BGSprite = new BGSprite('parkBG/parkfg', -1000, -500);
				add(fgSnow);
				
			case 'parkEvil': //Secret - Bad Boy
				var bg:BGSprite = new BGSprite('parkBG/parkbackevil', -1000, -300, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				if (!ClientPrefs.lowQuality) {
					var bgEscalator:BGSprite = new BGSprite('parkBG/parkbenchevil', -1000, -300, 0.3, 0.3);
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					add(bgEscalator);
				}

				var fgSnow:BGSprite = new BGSprite('parkBG/parkfgevil', -1000, -500);
				add(fgSnow);
		}
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'secret':
						if(isStoryMode && campaignMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice) {
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName)
							{
								case 'week2':
									if (achievementName == 'secret') unlock = true;
							}
						}
				}
